...
const response = await axios.post('http://localhost:5000/api/auth/google', { token: idToken });        console.log('Login response received:', response.data);
    ...
    const handleGoogleLogin = async () => {
      try {
        ...
        localStorage.setItem('token', response.data.token);
        localStorage.setItem('role', response.data.role);
        setError('Login successful with Google! Redirecting to home...');
        setTimeout(() => {
          onClose();
          navigate('/');
        }, 1000);
    ...
    const handleGithubLogin = async () => {
      try {
        ...
        localStorage.setItem('token', response.data.token);
        localStorage.setItem('role', response.data.role);
        setError('Login successful with GitHub! Redirecting to home...');
        setTimeout(() => {
          onClose();
          navigate('/');
        }, 1000);
